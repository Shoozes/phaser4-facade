...
assert.equal(html.style.getPropertyValue("overflow-x"), "hidden");
assert.equal(html.style.getPropertyValue("overflow-y"), "hidden");
assert.equal(html.style.getPropertyValue("overscroll-behavior"), "");
assert.equal(html.style.getPropertyValue("overscroll-behavior-x"), "none");
assert.equal(html.style.getPropertyValue("overscroll-behavior-y"), "none");
assert.equal(body.style.getPropertyValue("touch-action"), "none");
assert.equal(canvas.style.getPropertyValue("display"), "block");
...
assert.throws(() => createFullscreenHost({
    document: {
        documentElement: { style: invalidStyle },
        body: { style: invalidStyle },
        getElementById() { return null; },
        querySelector() { throw new SyntaxError("invalid selector"); }
    }
}, "#["), /invalid selector/);
assert.equal(invalidSelectorWrites, 0, "invalid parent selectors must fail before any style mutation");
